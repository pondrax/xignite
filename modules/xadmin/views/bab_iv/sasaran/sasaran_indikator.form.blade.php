<form action="@_path/update/" method="post" enctype="multipart/form-data" class="py-3 mt-3 border-top" novalidate>
  <div class="row pb-2">
    ...
    <div class="col form-group">
      <label>Sasaran</label>
      <input type="text" name="id_sasaran" class="form-control selectize" placeholder="Pilih Sasaran" value="{{$data->id_sasaran|''}}" data-options="daftar_sasaran">
    </div>
    <div class="w-100"></div>
    <div class="col-6 form-group">
      <label>Indikator</label>
      <textarea class="form-control h-min" name="indikator" placeholder="Sasaran" >{{$data->indikator|''}}</textarea>
    </div>
    <div class="col-6">
      <label>Target</label>
...
<script>
  Module.form.setSelectize({
    daftar_sasaran : {{jsonform($daftar_sasaran)}}
  });
  function kalkulasi(){
