...
class Pengisi_controller extends CI_Controller {
  function __construct() {
    ...
    $this->load->model('xadmin/bab_vii/pengisi');
    $this->data=[
      'path'=>base_url('xadmin/bab_vii/pengisi'),
  ...
  public function view($json=null,$deleted_filter=false){
    if($json){
      $this->data=Pengisi::select('users.id,instansi,master__periode.id,tahun,bab_vii__pengisi.*')
                         ->join('tahun')
                         ->join('pengguna')
      ...
      $this->load->blade('xadmin/bab_vii/pengisi/pengisi',$this->data);
    }
  }
  ...
  public function form($mode=''){
    ...
    foreach($data as $i=>$d){
      ...
      $this->data['data']=$d;
      $this->load->blade('xadmin/bab_vii/pengisi/pengisi.form',$this->data);
    }
  }
